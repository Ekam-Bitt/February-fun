...
const Page = () => {
  ...
  return (
    <main className="flex items-center justify-center flex-col w-screen h-screen absolute bg-pink-400">
      {view && (
        <>
          <Image src={kiss_bear} alt="bear-kiss" height={400} width={400} />
          <div className="fixed bottom-5 m-0 p-0 font-mono font-semibold">
            made by ekam
          </div>
        </>
      )}
      {!view && (
        <div className="flex flex-col w-full items-center absolute">
          ...
          />
          <motion.span
            animate={{ scale: 1.0 + scale * 0.03 }}
            className="text-center w-full text-white font-semibold text-2xl"
          >
